...
void IfstreamFunction::execute(SLIInterpreter *i) const
{
  ...
  i->require_type(0,sli::stringtype);
  ...
  if(in->good())
    {
      SLIistream stream= new SLIistream(in);
      
      i->top()=i->new_token<sli3::ifstreamtype>(stream);
      i->push(true);
    }
  else
    i->push(false);
...
void OfstreamFunction::execute(SLIInterpreter *i) const
{
  ...
  std::ostream *in = new ofdstream(sd->c_str());
  i->top().clear();
  if(in->good())
    {
      SLIostream stream= new SLIostream(in);
      
      i->top()=i->new_token<sli3::ofstreamtype>(stream);
      i->push(true);
    }
  else
    i->push(false);
...
void init_sli_io(SLIInterpreter *i)
{
  ...
  // ios dictionary.
  Token t_cin= i->new_token<sli3::istreamtype>( new SLIistream(std::cin));
  Token t_cout= i->new_token<sli3::ostreamtype>( new SLIostream(std::cout));
  Token t_cerr= i->new_token<sli3::ostreamtype>( new SLIostream(std::cerr));
